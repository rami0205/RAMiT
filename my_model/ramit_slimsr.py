# RAMiT-slimSR (Reciprocal Attention Mixing Transformer)
...
class Bottleneck(nn.Module):
    def __init__(self, dim, num_stages, act_layer=nn.GELU, norm_layer=ReshapeLayerNorm, mv_ver=1, mv_act=nn.LeakyReLU, exp_factor=1.2, expand_groups=4):
        ...
        if mv_ver==1:
            ...
            self.mobivari = MobiVari2(self.cat_dim, 3, 1, act=mv_act, out_dim=dim, exp_factor=2., expand_groups=1)
        self.act = act_layer()
        self.norm = norm_layer(dim)
...
class RAMiT(nn.Module):
    def __init__(self, in_chans=3, dim=64, depths=(6,4,4,6), num_heads=(4,4,4,4), head_dim=None,
                 chsa_head_ratio=0.25, window_size=8, hidden_ratio=2.0, qkv_bias=True, mv_ver=1, exp_factor=1.2, expand_groups=4,
