...
# NGswin: N-Gram in Swin Transformers for Efficient Lightweight Super-Resolution https://arxiv.org/abs/2211.11436
...
def half_learning_rate(optimizer, epoch, args, warm_start=False):
    ...
    # warm-start phase or not (only for SR x3, x4)
    if args.finetune and args.warm_start and (epoch+1) < args.warm_start_epoch+1: 
        return args.init_lr
    
    # scratch or whole-finetune phase
    epoch = epoch if epoch < args.lr_cycle else epoch-args.lr_cycle
    if epoch < args.warmup_epoch:
        lr = args.init_lr * epoch / args.warmup_epoch
    else:
...
def param_groups_lrd(model, weight_decay=0.05, no_weight_decay_list=[], layer_decay=.75, model_name='NGswin'):
    ...
    """
#     import pickle
#     with open('param_groups.pickle', 'wb') as f:
#         pickle.dump(param_groups, f)
    if weight_decay == 0.0 and layer_decay == 1.0:
        temp = dict([('lr_scale', 1.0), ('weight_decay', 0.0), ('params', list()), ('param_names', list())])
        param_groups = {'null': temp}
        for n,p in model.named_parameters():
            param_groups['null']['params'].append(p)
            param_groups['null']['param_names'].append(n)
